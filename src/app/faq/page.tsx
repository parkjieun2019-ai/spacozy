...
export default function FaqPage() {
  return (
    <>
            ...
            {faqs.map((f) => (
              <details key={f.q} className="group py-2">
                <summary className="flex cursor-pointer list-none items-start gap-4 py-5 [&::-webkit-details-marker]:hidden">
                  <span className="font-display text-[1.4rem] leading-none text-wood">Q</span>
                  <span className="flex-1 font-serif text-[1.12rem] leading-[1.6] text-ink">{f.q}</span>
                  <span className="mt-1.5 text-green transition-transform group-open:rotate-45" aria-hidden>
      ...
      <section id="location" className="scroll-mt-24 bg-beige/50 py-16 md:py-24">
        <Container className="grid gap-10 md:grid-cols-2 md:gap-16">
          <div>
            <Eyebrow className="mb-4">Location</Eyebrow>
            <h2 className="font-serif text-[1.6rem] text-green md:text-[2rem]">오시는 길</h2>
            <dl className="mt-8 space-y-6">
              <div>
                <dt className="text-sm text-greige">주소</dt>
                <dd className="mt-1 text-[1.05rem]">{site.address}</dd>
              </div>
              <div>
                <dt className="text-sm text-greige">예약 문의</dt>
              ...
              <div>
                <dt className="text-sm text-greige">영업시간</dt>
                <dd className="mt-1 space-y-0.5 text-[1.05rem]">
                  {site.hours.map((h) => (
                    <p key={h.day}>
                      <span className="inline-block w-20 text-greige">{h.day}</span> {h.time}
                    </p>
                  ))}
                </dd>
              </div>
              <div>
                <dt className="text-sm text-greige">주차</dt>
          ...
          >
            ...
            <div className="relative text-center">
              <svg className="mx-auto h-12 w-12 text-green" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                <path d="M12 2a7 7 0 0 0-7 7c0 5.25 7 13 7 13s7-7.75 7-13a7 7 0 0 0-7-7Zm0 9.5A2.5 2.5 0 1 1 12 6.5a2.5 2.5 0 0 1 0 5Z" />
              </svg>
              <p className="mt-3 font-serif text-[1.15rem] text-green">죽전누리에뜰 B동 2층</p>
