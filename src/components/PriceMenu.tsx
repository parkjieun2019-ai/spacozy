...
/**
 * 카테고리 탭 + 세부 프로그램 가격표
 * 모든 탭의 내용을 HTML에 미리 넣어 두고(hidden 처리) 검색엔진·AI 도구도 전부 읽을 수 있게 합니다.
 */
export default function PriceMenu({ ids }: { ids?: string[] }) {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      {/* 가격표 — 모든 카테고리를 그려 두고 선택된 것만 보여줍니다 */}
      {list.map((cat) => (
        <div
          key={cat.id}
          id={`panel-${cat.id}`}
          role="tabpanel"
          aria-labelledby={list.length > 1 ? `tab-${cat.id}` : undefined}
          hidden={cat.id !== active}
          className={`fade-up ${list.length > 1 ? "mt-6" : ""} rounded-[18px] border border-line bg-paper p-5 md:p-8`}
        >
              ...
              {cat.items.map((it) => (
                <li key={it.name} className="flex flex-col gap-3 py-5 sm:flex-row sm:items-center sm:gap-6">
                  <div className="flex-1">
                    <p className="flex flex-wrap items-center gap-2 text-[1.08rem] font-semibold text-ink">{it.name}</p>
                    <p className="mt-1 text-[0.9rem] text-muted">{[it.desc, it.duration].filter(Boolean).join(" · ")}</p>
                  </div>
